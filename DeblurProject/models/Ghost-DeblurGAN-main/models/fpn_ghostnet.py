...
class FPN(nn.Module):
    ...
    def __init__(self, norm_layer, num_filters=128, pretrained=True):
        ...
        # 修改这部分，添加错误处理，防止无法下载预训练模型
        try:
            print("尝试加载GhostNet预训练模型...")
            model = timm.create_model('ghostnet_100', pretrained=pretrained, features_only=True)
            print("成功加载GhostNet预训练模型")
        except Exception as e:
            print(f"无法下载预训练模型，使用随机初始化模型: {str(e)}")
            model = timm.create_model('ghostnet_100', pretrained=False, features_only=True)
            print("已使用随机初始化模型")
